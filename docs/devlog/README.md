...
| 김윤한 | — | [→](members/김윤한.md) |
| 반소람 | — | [→](members/반소람.md) |
| 정환주 | 시연영상 TTS 더빙 파이프라인 — 대본 데이터(`scenario_lines.py`)·일괄 합성 스크립트(`synth_scenario.py`)·생성 가이드 정리, 긴 편지 후반 음량 소실(RMS 측정) 대응 문장단위 분할+RMS 중앙값 정규화, 시연 전용 `woman_bright` 보이스 추가(서비스 narration 불변) | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
| 장민수 | — | [→](members/장민수.md) |
