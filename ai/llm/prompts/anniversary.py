...
# 트리거 기념일 목록.
MILESTONE_DAYS: Final[tuple[int, ...]] = (30, 100, 365)
...
MILESTONE_TEMPLATES: Final[dict[int, str]] = {
    ...
    365: (
        "{name}이/가 무지개다리를 건넌 지 1년이 되었습니다. "
        "한 해를 보내며 계절이 바뀌는 동안에도, 보호자님은 {name}을/를 마음 깊이 "
        "기억해 오셨습니다.\n\n"
        "{name}과/와 함께한 시간은 1년이 지난 지금도 사라지지 않고, 보호자님의 "
        "하루하루 속에 자연스럽게 스며들어 있습니다. 문득 떠오르는 기억들은 "
        "슬픔이기도 하지만, 그만큼 사랑했다는 증거이기도 합니다.\n\n"
        "1주기를 맞아 {name}과/와의 추억을 천천히 떠올려보는 시간을 가져보셔도 "
        "좋겠습니다. 지난 1년을 잘 지내오신 보호자님을, 그리고 앞으로의 시간도 "
        "함께 응원하겠습니다."
    ),
}
...
MILESTONE_FOCUS: Final[dict[int, str]] = {
    ...
    365: (
        "{name}이/가 떠난 지 1년(1주기)이 되었습니다. "
        "1년이라는 시간의 의미를 인정하며, {name}과의 기억이 보호자님 안에 "
        "여전히 따뜻하게 남아있음을 전해 주세요. "
        "근거 없이 '이제 괜찮아질 거예요'라고 말하기보다, "
        "함께 걸어온 1년과 앞으로도 함께할 시간을 담아 위로해 주세요."
    ),
}
