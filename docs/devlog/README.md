...
| AI/LLM | 반소람·정환주 | 5/5 + 부가 모듈 완성 | 회복점수 4축 산식 확정·교체 완료 (PR #284·#286), 프롬프트 전체 최종본 동결(06-15) |
...
| ④ TTS 엔진 (Qwen3) | 정환주 | ✅ |
| ⑧ 평가 지표/집계 | 정환주 | ✅ |
| 회복 지수 재설계 | 반소람·모세종 | ✅ 4축 산식 확정·백엔드 교체 완료 (PR #284·#286) |
...
| 정환주 | — | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
| 장민수 | Gemini TTS 전환(`_gemini_tts.py`), MuseTalk→LP 재현, 립싱크 desync 트러블슈팅(원인 전수 배제 → 로컬 MuseTalk TTS 미반영 진단), LP keypoint 우회책 채택(`wav_lipsync.py`, 입–에너지 corr ~0.84), 발표 자료 정리 | [→](members/장민수.md#2026-06-18-day-18) |
...
| 이름 | 한 일 (요약) | 상세 |
|------|------------|------|
| 모세종 | NCP 재시작 후 시드 재실행, demo04 voiced_url 복구, demo01 GIF+LP 생성, TTS CER(3인칭 1.0%/1인칭 5.5%) + 립싱크(corr 0.896/lag +40ms) 자체 평가 완료, PR #365 OTA 배포 | [→](members/모세종.md#2026-06-17-day-17) |
| 김윤한 | — | [→](members/김윤한.md) |
| 반소람 | — | [→](members/반소람.md) |
| 정환주 | — | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
...
**팀 전체:** 강사님 MuseTalk→LivePortrait 파이프라인 공유 수령. TTS·립싱크 자체 평가 전 항목 PASS. 데모 계정 전체 재정상화.
...
| 김윤한 | — | [→](members/김윤한.md) |
| 반소람 | — | [→](members/반소람.md) |
| 정환주 | TTS 로그분석기 신설(서버 무중단·엔진별 사용량/비용 요약·CSV/MD 제출), STT 발음 객관평가(CER 0%/WER 9.1%·제출 JSON), PR #336 리뷰·승인, Qwen3 운영 pm2 전환·실측검증(EINVAL→cmd.exe /c)·수동운영 환원, TTS 실행 인수인계 가이드 | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
| 장민수 | 1인칭 LP 생성 오류 진단(변수명 불일치, dev에 이미 수정 반영 확인) + dev 최신화, 발표 시연 영상 소스 정리(데모펫 사진·BGM·driving 영상), driving 최종 확정값(d3.mp4+0.5) 재확인 및 fallback 불일치 발견, SyncNet 평가 요청 대응 | [→](members/장민수.md#2026-06-16-day-16) |

**팀 전체:** TTS 운영 가시성·품질 측정 도구 마련(로그 분석기·엔진 비용 요약·STT 발음 평가). Qwen3 GPU 서버 운영을 pm2로 정리(수동 운영 확정) + 실행 인수인계 가이드 작성.
...
**팀 전체:** 회복 게이트 4축 산식 교체 완료 (PR #286). 소람님 AI 대형 PR #284 머지 (감정추론·폰사용 분석·삼성헬스). 정환주님 TTS URL 자동 핸드오프 완성 (PR #285). evaluate.py 평가 스크립트 구비.
...
| 모세종 | — | [→](members/모세종.md) |
| 김윤한 | — | [→](members/김윤한.md) |
| 반소람 | 발표 PPT 슬라이드 코드 검증(슬라이드 10~17), 발표 준비 메모 작성, 스크립트 Q&A 보강(감정추세·생활패턴), 위기 감지 점수제·테스트셋 심화 분석 및 발표 캡처 포인트 도출 | [→](members/반소람.md) |
| 정환주 | — | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
| 장민수 | — | [→](members/장민수.md) |
...
| 모세종 | — | [→](members/모세종.md) |
| 김윤한 | — | [→](members/김윤한.md) |
| 반소람 | AI휴먼 정량평가 체크리스트 완성(§0~§8 구조 정리·담당자별 현행화·최종 평가표 대화 지표 취합: G-Eval 전 항목 ≥4.6·CER 1.0%·corr 0.896), 발표 PPT 수정 | [→](members/반소람.md) |
| 정환주 | — | [→](members/정환주.md) |
| 민경이 | — | [→](members/민경이.md) |
| 장민수 | — | [→](members/장민수.md) |
...
**팀 전체:** TTS WaveSpeedAI 메인 전환 완료. 민경이님 회복 여정 선물함 UI 완성 (PR #279). 한글 조사 유틸 도입으로 이름 표기 일관화. PR 총 286건.
